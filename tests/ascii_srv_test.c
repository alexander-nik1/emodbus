...
static emb_ba_word_t coils1_data[0x10000/sizeof(emb_ba_word_t)];

static uint8_t coils1_read(struct emb_srv_bits_t* _coils,
                           uint16_t _offset,
                           uint16_t _quantity,
                           uint8_t* _pvalues)
{
    (void)_coils;
    return (uint8_t)emb_bit_arr_get_bits(coils1_data, sizeof(coils1_data), _pvalues, _offset, _quantity);
}

static uint8_t coils1_write(struct emb_srv_bits_t* _coils,
                            uint16_t _offset,
                            uint16_t _quantity,
                            const uint8_t* _pvalues)
{
    (void)_coils;
    return (uint8_t)emb_bit_arr_set_bits(coils1_data, sizeof(coils1_data), _pvalues, _offset, _quantity);
}

static struct emb_srv_bits_t coils1 =
{
    .start = 0,
    .size = sizeof(coils1_data)*sizeof(coils1_data[0]),
    .read_bits = coils1_read,
    .write_bits = coils1_write
};
...
{
    (void)_srv;
    if(emb_is_addr_belongs_to_bits(_begin, &coils1))
        return &coils1;
    return NULL;
}
...
int main()
{
    ...
    while(1) {
        ...
        int tmp;
        tmp = emb_serial_port_receive_ascii(&serial_port, buf, sizeof(buf));
        if(tmp < 0) {
