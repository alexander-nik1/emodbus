...
int main(int argc, char* argv[]) {
    ...
    struct emb_rtu_via_serial_t* mb_serial;

    if ( argc < 2 )
    {	
	printf("Error: EModbus server missing serial port name.\n" );
	return 1;
    }
	
    printf("EModbus server example with serial port transport system\n" );
    ...
    mb_serial = emb_rtu_via_serial_create(base, 10, argv[ 1 ], 115200);
    if(!mb_serial) {
        fprintf(stderr, "Error: emb_rtu_via_serial_create (port: %s) return NULL\n", argv[ 1 ]);
        fflush(stderr);
        goto ret;
    }

    emb_super_server_init(&mb_ssrv);
    emb_super_server_set_transport(&mb_ssrv, emb_rtu_via_serial_get_transport(mb_serial));

    emb_rtu_via_serial_get_transport(mb_serial)->flags |= EMB_TRANSPORT_FLAG_DUMD_PAKETS;
...
ret:
    ...
    if(mb_serial)
        emb_rtu_via_serial_destroy(mb_serial);
    if(base)
