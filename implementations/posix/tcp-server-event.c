...
{
    struct tcp_server_event_t* ctx = (struct tcp_server_event_t*)_ctx;
...
struct tcp_server_event_t* tcp_server_new(struct event_base* _base,
                                    tcp_server_notifier_t _event_notifier,
                                    int _port) {

    struct tcp_server_event_t* res = (struct tcp_server_event_t*)malloc(sizeof(struct tcp_server_event_t));
    if(res) {

        memset(res, 0, sizeof(struct tcp_server_event_t));
        res->base = _base;
...
void tcp_server_free(struct tcp_server_event_t* _ctx) {
    if(_ctx) {
...
int tcp_server_read(struct tcp_server_event_t* _ctx,
                    void *_client_id,
                    void* _p_buffer,
                    size_t _buff_size) {
    if(_ctx) {
...
int tcp_server_write(struct tcp_server_event_t* _ctx,
                     void *_client_id,
                     const void* _p_data,
                     size_t _data_size) {
    if(_ctx) {
...
void tcp_server_set_user_data(struct tcp_server_event_t* _ctx, void* _user_data) {
    _ctx->user_data = _user_data;
}

void* tcp_server_get_user_data(struct tcp_server_event_t* _ctx) {
    return _ctx->user_data;
}

void tcp_server_set_clients_limit(struct tcp_server_event_t* _ctx,
                                  int _clients_limit) {
    if(_ctx)
        _ctx->n_clients_limit = _clients_limit;
}

int tcp_server_clients_limit(const struct tcp_server_event_t* _ctx) {
    if(_ctx)
